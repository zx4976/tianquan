...
class BookIndex:
    ...
    def _build_schema(self):
        ...
        sb.add_text_field("body", stored=True)        # 分词后正文（用于搜索）
        sb.add_text_field("body_raw", stored=True)    # 原始正文（用于精读）
    ...
    def add_book(self, book_id, title, author, category, body, body_raw="",
                 tags="", lang="", tokenized=False):
        """添加一本书。body 为分词正文，body_raw 为原始正文。"""
        if not self.index:
        ...
        doc.add_text("body_raw", body_raw)  # 存原始正文
    ...
    def add_books_batch(self, books, tokenized=False):
        ...
        for book in books:
            ...
            body_raw = str(book.get("body_raw", ""))
            ...
            if tokenized:
                ...
                doc.add_text("body", tokenize_text(body))
                doc.add_text("tags", tokenize_text(tags))
            doc.add_text("body_raw", body_raw)
    ...
    def get_body(self, book_id):
        """按 book_id 获取分词后正文（用于搜索）"""
        return self._get_field(book_id, "body")

    def get_body_raw(self, book_id):
        """按 book_id 获取原始正文（用于精读）"""
        return self._get_field(book_id, "body_raw")
    ...
    def _get_field(self, book_id, field_name):
        ...
        try:
            ...
            if hits:
                ...
                return doc.get_first(field_name) or ''
        except Exception:
            pass
        return ''
        ...
        try:
            ...
            if hits:
                ...
                return {
                    ...
                    'category': doc.get_first("category") or "",
                    'lang': doc.get_first("lang") or "",
                    'body_chars': len(doc.get_first("body_raw") or ""),
                }
        except Exception:
            pass
        return None
