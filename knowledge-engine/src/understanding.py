...
UNDERSTANDING_DB = os.path.expanduser("~/projects/knowledge-engine/data/understanding.db")
...
class Understanding:
    ...
    def _init_db(self):
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS comprehensions (
                ...
                evidence_offset INTEGER DEFAULT 0,
                importance INTEGER DEFAULT 5,
                created_at REAL,
                FOREIGN KEY (book_id) REFERENCES books(id)
            )
        """)
        # 兼容旧表：如果 evidence 列不存在则添加
        try:
            conn.execute("ALTER TABLE comprehensions ADD COLUMN evidence TEXT DEFAULT ''")
        except sqlite3.OperationalError:
            pass
        try:
            conn.execute("ALTER TABLE comprehensions ADD COLUMN evidence_offset INTEGER DEFAULT 0")
        except sqlite3.OperationalError:
            pass
        # 可执行知识：定理/公式/定义，可用于后续推理和计算
        conn.execute("""
            CREATE TABLE IF NOT EXISTS knowledge (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                book_id TEXT,
                name TEXT NOT NULL,
                ktype TEXT DEFAULT '定理',
                symbol TEXT DEFAULT '',
                latex TEXT DEFAULT '',
    ...
    def add_comprehension(self, book_id, dimension, content, importance=5, evidence="", evidence_offset=0):
        """添加一条理解记录，带原文证据"""
        conn = sqlite3.connect(self.db_path)
        conn.execute("""
            INSERT INTO comprehensions (book_id, dimension, content, evidence, evidence_offset, importance, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (book_id, dimension, content, evidence, evidence_offset, importance, time.time()))
        conn.commit()
        conn.close()
    ...
    def add_knowledge(self, book_id, name, ktype="定理", symbol="", latex="",
                      description="", code="", dependencies=None):
        """存储可执行知识（定理/公式/定义），供后续推理调用"""
        conn = sqlite3.connect(self.db_path)
        conn.execute("""
            INSERT INTO knowledge (book_id, name, ktype, symbol, latex, description, code, dependencies, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (book_id, name, ktype, symbol, latex, description, code,
              json.dumps(dependencies or []), time.time()))
        conn.commit()
        conn.close()

    def get_knowledge(self, book_id=None, ktype=None):
        """获取可执行知识"""
        conn = sqlite3.connect(self.db_path)
        where = []
        params = []
        ...
        rows = conn.execute(
            f"SELECT name, ktype, symbol, latex, description, code, dependencies FROM knowledge {w} ORDER BY created_at",
            params
        ).fetchall()
        conn.close()
        return [{"name":r[0],"ktype":r[1],"symbol":r[2],"latex":r[3],
                 "description":r[4],"code":r[5],
                 "dependencies":json.loads(r[6]) if r[6] else []} for r in rows]

    def search_knowledge(self, keyword):
        """搜索可执行知识"""
        conn = sqlite3.connect(self.db_path)
        rows = conn.execute(
            "SELECT name, ktype, symbol, latex, description FROM knowledge WHERE name LIKE ? OR symbol LIKE ? OR description LIKE ? LIMIT 10",
            (f"%{keyword}%", f"%{keyword}%", f"%{keyword}%")
        ).fetchall()
        conn.close()
        return [{"name":r[0],"ktype":r[1],"symbol":r[2],"latex":r[3],"description":r[4]} for r in rows]

    def add_inference_rule(self, name, rule_type, pattern, replacement, condition="", description=""):
        """存储推理规则"""
        conn = sqlite3.connect(self.db_path)
        conn.execute("""
            INSERT INTO inference_rules (name, rule_type, pattern, replacement, condition, description, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (name, rule_type, pattern, replacement, condition, description, time.time()))
        conn.commit()
        conn.close()
    ...
    def get_comprehensions(self, book_id):
        ...
        conn = sqlite3.connect(self.db_path)
        rows = conn.execute(
            "SELECT dimension, content, evidence, evidence_offset, importance FROM comprehensions WHERE book_id=? ORDER BY importance DESC",
            (book_id,)
        ).fetchall()
        conn.close()
        return [{'dimension': r[0], 'content': r[1], 'evidence': r[2] or '',
                 'evidence_offset': r[3] or 0, 'importance': r[4]} for r in rows]
